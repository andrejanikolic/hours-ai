...
/** True when we parsed at least 1 venue and every result is a no-op. */
  ...
  <div class="venues">
    ...
    <template v-else>
      ...
      <section v-if="previews.length" class="previews">
        <!-- Shortcut banner: nothing actually changed for ANY venue -->
        <div v-if="allNoChange" class="no-changes">
          ...
          <div class="no-changes__text">
            ...
            <p>
              ...
              {{ previews.length === 1 ? 'venue' : 'venues' }} — nothing has changed.
            </p>
          </div>
        ...
        <template v-else>
          ...
          >
            ...
            <p v-else-if="p.noChange" class="preview-card__no-change">
              HoursAI returned the same schedule that's already saved — nothing to update for this venue.
            </p>
            <ServingTimesDiff
              v-else
              :current="p.current"
...
@media (max-width: 640px) {
  ...
  .previews__actions :deep(.btn), .previews__actions > * { flex: 1; }

  .preview-card { padding: 14px; }
  .preview-card__head { flex-wrap: wrap; }
  .preview-card__status { margin-left: 0; }
}
</style>
